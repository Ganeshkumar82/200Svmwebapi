...
//         admin.sitecontrollerpath = "\\\\192.168.0.155\\Sitecontrollers";
//       }
//       if (admin.hotlinestorage == null || admin.hotlinestorage == "") {
//         admin.hotlinestorage = "\\\\192.168.0.155\\Volumes1";
//       }
...
async function addcompanysite(admin) {
  try {
    const sites = Array.isArray(admin) ? admin : [admin];
    const results = await Promise.all(sites.map(processSite));
    return results;
  } catch (error) {
    console.error("Error in addCompanySite:", error.message);
    return {
      code: false,
      message: "Customer site add failed",
      error: error.message,
...
async function processSite(site) {
  ...
  const { sitename, sitecontrollerpath, hotlinestorage, archivalpath } =
    sanitizePaths(site);
  const sitefolderPath = path.join(sitecontrollerpath, sitename);
  ...
  try {
    const [result] = await db.spcall(
      ...
      [
        ...
        site.branchcode,
        site.gstno,
        site.whatsappgroupname,
        site.sitetype,
      ]
    );

    const branchId = result[1][0]["@result"];
    if (!branchId) {
      return {
        code: false,
        message: "Customer site add failed",
    ...
    const folderPath = await createFolder(sitecontrollerpath, sitename);
    await addDepartment(branchId, site.sitename);
    await copyFiles(config.sitecontrollerpath, folderPath);
    ...
    console.error("Error in processSite:", error.message);
    return { code: false, message: error.message, value: site.customerid };
  }
}
...
function validateSite(site) {
  ...
  for (const field of requiredFields) {
    if (!site.hasOwnProperty(field)) {
      return {
        isValid: false,
        result: {
          code: false,
          message: `${field} is missing. Please provide a valid ${field}.`,
          value: site.customerid,
...
// Helper function to sanitize paths and handle default values
function sanitizePaths(site) {
  const sitename = site.sitename.replace(/\s/g, "").replace(/[^\w\s]/gi, "");
  return {
    sitename,
    sitecontrollerpath:
      site.sitecontrollerpath || "\\\\192.168.0.155\\Sitecontrollers",
    hotlinestorage: site.hotlinestorage || "\\\\192.168.0.155\\Volumes1",
    archivalpath: site.archivalpath || "\\\\192.168.0.198\\Volumes2",
  };
}

// Helper function to create a folder
async function createFolder(basePath, sitename) {
  const folderPath = path.join(basePath, sitename);
  try {
...
// Helper function to add department
async function addDepartment(branchId, sitename) {
  try {
    const [result] = await db.spcall(
      "CALL SP_SITE_DEPT_ADD(?,?,?,?,@result); select @result;",
      [branchId, sitename, sitename, 0]
    );
    console.log(`Department added with ID: ${result[1][0]["@result"]}`);
  } catch (error) {
...
async function copyFiles(source, destination) {
  try {
    await cpy.copy(source, destination);
    console.log("Files copied successfully!");
  } catch (error) {
...
async function addsiteconfig(siteid) {
  try {
    ...
    sc.Mqtt_Port MqttPort,sc.Api_server apiserver,sc.Nginxserver1 nginxserver1,sc.Nginxserver2 nginxserver2,sc.ArchivalTime ArchivalTime,SC.svmAI_Path SVMAiPath,sc.aiserver from branchmaster bm, databasemaster db,siteconfigmaster sc where bm.Branch_ID = ${siteid} and bm.status= 1 and 
      ...
      try {
        ...
        if (deviceDetailsArray.length >= 1) {
          ...
          if (deviceInfo.sdkid == "Dahua") {
            ...
            sdkid = 1;
            brand = 3;
          } else if (deviceInfo.sdkid == "TVT") {
            sdkid = 4;
            brand = 4;
          } else if (deviceInfo.sdkid == "UNV") {
            sdkid = 5;
            brand = 5;
          } else {
  ...
  try {
    ...
    for (const deviceInfo of adminArray) {
      ...
      try {
        ...
        if (deviceInfo.hasOwnProperty("secondaryip") == false) {
          // return helper.getErrorResponse('DEVICE_PASSWORD_MISSING', "Device password missing. Please provide the Device password.","ADD DEVICE","");
          deviceDetailsArray.push({
            code: false,
            message: "Secondary IP missing. Please provide the secondary ip.",
            id: 0,
            name: deviceInfo.devicename,
          });
          continue;
...
async function processEvent(event, siteid, deptid) {
  ...
  const TVTError = event?.TVTInfo?.Error;
  const UNVError = event?.UNVInfo?.Error;
  const deviceid =
    event?.HikInfo?.DeviceID ||
    event?.DahuaInfo?.DeviceID ||
    event?.TVTInfo?.DeviceID ||
    event?.UNVInfo?.DeviceID;
  // const dahuaError = event?.DahuaInfo?.Error;

  if (!hikError && !dahuaError && !TVTError && !UNVError) {
    try {
    ...
    deviceDetailsArray.push({
      code: false,
      message: hikError || dahuaError || TVTError || UNVError,
      id: deviceid,
      name: devicename,
    });
  }
...
async function processDeviceInfo(item, siteid, deptid) {
  try {
    ...
    if (!item.DahuaInfo && !item.HikInfo && !item.TVTInfo && !item.UNVInfo) {
      console.error("No information available for device.");
      return;
    }
    ...
    for (const key in item) {
      ...
      if (info) {
        ...
        serialNumber = info.SerialNumber ?? deviceId;
    ...
    const sql = `UPDATE devicemaster SET SerialNo = ?, Model_no = ?,Device_type = ?,Product_name = ?, No_AnalogCH = ?, No_IpCH = ?, Harddisk = ?, harddiskcap = ?, harddiskfreespace = ?, software_version = ?, status = 1 WHERE Device_id = ?`;
    await db.query(sql, [
      serialNumber,
      devicemodel,
      devicetype,
    ...
    const sql1 = await db.query(
      `select Dept_id,Branch_id from deptmaster where Dept_id IN(select Dept_id from devicemaster where Device_id = ?) LIMIT 1`,
      [deviceId]
    );
    if (sql1[0]) {
  ...
  try {
    ...
    for (let i = 0; i < channelList.length; i++) {
      ...
      try {
        if (channelType === "ip") {
          var channelName;
          if (channelList.length == 64) {
            var chltype = 1;
            channelName = channelNumber ? `channel${channelNumber}` : "";
          } else {
            var chltype = 1;
            channelName = channelNumber ? `channel${channelNumber + 32}` : "";
          }
          const [result] = await db.spcall(
...
async function getsitelist(admin) {
  try {
    ...
    if (
      ...
      CASE WHEN bm.sitecontroller_status = 1 THEN DATE_FORMAT(bm.site_uptime, '%Y-%m-%d %H:%i:%s') ELSE DATE_FORMAT(bm.site_downtime, '%Y-%m-%d %H:%i:%s') END AS time,
       bm.snapcount, bm.Sitecontroller_path, bm.Branch_name, bm.status AS Sitestatus, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime,
      COUNT(dm.Device_id) AS Total_Devices, bm.branch_code, bm.gstno, SUM(CASE WHEN dm.runningstatus = 1 THEN 1 ELSE 0 END) AS Active_Devices,
      SUM(CASE WHEN dm.runningstatus = 0 THEN 1 ELSE 0 END) AS Inactive_Devices, bm.hidestatus,CASE WHEN cst.branch_id IS NOT NULL THEN 1 ELSE 0 END AS subscription,bm.Branch_Logo FROM branchmaster bm LEFT JOIN deptmaster dt ON bm.Branch_id = dt.Branch_id 
      LEFT JOIN devicemaster dm ON dt.Dept_id = dm.Dept_id AND dm.Deleted_Flag = 0 LEFT JOIN subscriptioncustomertrans cst ON cst.branch_id = bm.Branch_id AND bm.Customer_ID != 0 WHERE bm.Deleted_Flag = 0 AND bm.site_type = ${admin.sitetype} GROUP BY bm.Branch_id, bm.Customer_id,
      bm.contact_person, bm.Address, bm.city, bm.state, bm.pincode, bm.site_starttime, bm.site_endtime, bm.Email_id, bm.Contact_no, bm.whatsappgroupname,
      bm.Image_hotlinepath, bm.Image_archivalpath, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, bm.Branch_name, bm.status,
      bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, bm.branch_code, bm.gstno, bm.hidestatus ORDER BY bm.Branch_id;`);
    } else if (
      ...
      bm.Sitecontroller_path, CASE WHEN bm.sitecontroller_status = 1 THEN DATE_FORMAT(bm.site_uptime, '%Y-%m-%d %H:%i:%s') ELSE DATE_FORMAT(bm.site_downtime, '%Y-%m-%d %H:%i:%s')
      END AS time, bm.Branch_name, bm.status AS Sitestatus, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, COUNT(dm.Device_id) AS Total_Devices, bm.branch_code, bm.gstno,
      SUM(CASE WHEN dm.runningstatus = 1 THEN 1 ELSE 0 END) AS Active_Devices, SUM(CASE WHEN dm.runningstatus = 0 THEN 1 ELSE 0 END) AS Inactive_Devices, bm.hidestatus,CASE WHEN cst.branch_id IS NOT NULL THEN 1 ELSE 0 END AS subscription,bm.Branch_Logo FROM branchmaster bm LEFT JOIN
      deptmaster dt ON bm.Branch_id = dt.Branch_id LEFT JOIN devicemaster dm ON dt.Dept_id = dm.Dept_id AND dm.Deleted_Flag = 0 LEFT JOIN subscriptioncustomertrans cst ON cst.branch_id = bm.Branch_id AND bm.Customer_ID != 0 WHERE bm.Deleted_Flag = 0 AND bm.status = 1 AND 
      bm.site_type = ${admin.sitetype} GROUP BY bm.Branch_id, bm.Customer_id, bm.contact_person, bm.Address, bm.city, bm.state, bm.pincode, bm.site_starttime, bm.site_endtime,
      bm.Email_id, bm.Contact_no, bm.whatsappgroupname, bm.Image_hotlinepath, bm.Image_archivalpath, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, bm.Branch_name,
      bm.status, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, bm.branch_code, bm.gstno, bm.hidestatus ORDER BY bm.Branch_id;
      `);
      ...
      bm.Site_Downtime, COUNT(dm.Device_id) AS Total_Devices,bm.branch_code,bm.gstno,
      SUM(CASE WHEN dm.runningstatus = 1 THEN 1 ELSE 0 END) AS Active_Devices, SUM(CASE WHEN dm.runningstatus = 0 THEN 1 ELSE 0 END) AS Inactive_Devices,bm.hidestatus,CASE WHEN cst.branch_id IS NOT NULL THEN 1 ELSE 0 END AS subscription,bm.Branch_Logo FROM branchmaster bm LEFT JOIN 
      deptmaster dt ON bm.Branch_id = dt.Branch_id LEFT JOIN devicemaster dm ON dt.Dept_id = dm.Dept_id AND dm.Deleted_Flag = 0 LEFT JOIN subscriptioncustomertrans cst ON cst.branch_id = bm.Branch_id and bm.Customer_ID != 0 where bm.Deleted_Flag = 0 AND bm.status = 0 AND bm.site_type = ${admin.sitetype} GROUP BY bm.Branch_id, 
      bm.Branch_name, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime;`);
    } else if (
      admin.filter == "ALL" &&
      admin.organizationid != 0 &&
      ...
      bm.whatsappgroupname, bm.Image_hotlinepath AS hotline, bm.Image_archivalpath AS Archival, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, CASE WHEN bm.sitecontroller_status = 1 
      THEN DATE_FORMAT(bm.site_uptime, '%Y-%m-%d %H:%i:%s') ELSE DATE_FORMAT(bm.site_downtime, '%Y-%m-%d %H:%i:%s') END AS time, bm.Branch_name, bm.status AS Sitestatus, bm.SiteController_Status,
      bm.Site_uptime, bm.Site_Downtime, COUNT(dm.Device_id) AS Total_Devices, bm.branch_code, bm.gstno, SUM(CASE WHEN dm.runningstatus = 1 THEN 1 ELSE 0 END) AS Active_Devices, SUM(CASE WHEN dm.runningstatus = 0 
      THEN 1 ELSE 0 END) AS Inactive_Devices, bm.hidestatus, CASE WHEN cst.branch_id IS NOT NULL THEN 1 ELSE 0 END AS subscription,bm.Branch_Logo FROM branchmaster bm LEFT JOIN deptmaster dt ON bm.Branch_id = dt.Branch_id LEFT JOIN devicemaster dm ON dt.Dept_id = dm.Dept_id AND dm.Deleted_Flag = 0 LEFT JOIN subscriptioncustomertrans cst ON cst.branch_id = bm.Branch_id AND
      bm.Customer_ID != 0 WHERE bm.site_type = ${admin.sitetype} AND bm.Customer_id IN (SELECT Customer_id FROM customermaster WHERE Organization_id = ${admin.organizationid}) GROUP BY bm.Branch_id, bm.Customer_id, bm.contact_person, bm.Address, bm.city, bm.state, bm.pincode, bm.site_starttime, bm.site_endtime,
      bm.Email_id, bm.Contact_no, bm.whatsappgroupname, bm.Image_hotlinepath, bm.Image_archivalpath, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, bm.Branch_name,
      bm.status, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, bm.branch_code, bm.gstno, bm.hidestatus ORDER BY bm.Branch_id;
      `);
    } else if (
      ...
      sql =
       ...
       bm.whatsappgroupname, bm.Image_hotlinepath AS hotline, bm.Image_archivalpath AS Archival, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, CASE WHEN bm.sitecontroller_status = 1
       THEN DATE_FORMAT(bm.site_uptime, '%Y-%m-%d %H:%i:%s') ELSE DATE_FORMAT(bm.site_downtime, '%Y-%m-%d %H:%i:%s') END AS time, bm.Branch_name, bm.status AS Sitestatus, bm.branch_code, bm.gstno,
       bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, COUNT(dm.Device_id) AS Total_Devices, SUM(CASE WHEN dm.runningstatus = 1 THEN 1 ELSE 0 END) AS Active_Devices, SUM(CASE WHEN
       dm.runningstatus = 0 THEN 1 ELSE 0 END) AS Inactive_Devices, bm.hidestatus,CASE WHEN cst.branch_id IS NOT NULL THEN 1 ELSE 0 END AS subscription,bm.Branch_Logo FROM branchmaster bm LEFT JOIN deptmaster dt ON bm.Branch_id = dt.Branch_id LEFT JOIN devicemaster dm ON dt.Dept_id 
       = dm.Dept_id AND dm.Deleted_Flag = 0 LEFT JOIN subscriptioncustomertrans cst ON cst.branch_id = bm.Branch_id AND bm.Customer_ID != 0 WHERE bm.Deleted_Flag = 0 AND bm.status = 0 AND bm.site_type = ${admin.sitetype} AND bm.Customer_id IN (SELECT Customer_id FROM customermaster WHERE
       Organization_id = ${admin.organizationid}) GROUP BY bm.Branch_id, bm.Customer_id, bm.contact_person, bm.Address, bm.city, bm.state, bm.pincode, bm.site_starttime, bm.site_endtime, 
       bm.Email_id, bm.Contact_no, bm.whatsappgroupname, bm.Image_hotlinepath, bm.Image_archivalpath, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, bm.Branch_name, bm.status,
       bm.branch_code, bm.gstno, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, bm.hidestatus ORDER BY bm.Branch_id;
      `);
      ...
      bm.whatsappgroupname, bm.Image_hotlinepath AS hotline, bm.Image_archivalpath AS Archival, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, CASE WHEN 
      bm.sitecontroller_status = 1 THEN DATE_FORMAT(bm.site_uptime, '%Y-%m-%d %H:%i:%s') ELSE DATE_FORMAT(bm.site_downtime, '%Y-%m-%d %H:%i:%s') END AS time, bm.Branch_name,
      bm.status AS Sitestatus, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, COUNT(dm.Device_id) AS Total_Devices, bm.branch_code, bm.gstno, SUM(CASE WHEN dm.runningstatus = 1 
      THEN 1 ELSE 0 END) AS Active_Devices, SUM(CASE WHEN dm.runningstatus = 0 THEN 1 ELSE 0 END) AS Inactive_Devices, bm.hidestatus,CASE WHEN cst.branch_id IS NOT NULL THEN 1 ELSE 0 END AS subscription,bm.Branch_Logo FROM branchmaster bm LEFT JOIN deptmaster dt ON
      bm.Branch_id = dt.Branch_id LEFT JOIN devicemaster dm ON dt.Dept_id = dm.Dept_id AND dm.Deleted_Flag = 0 LEFT JOIN subscriptioncustomertrans cst ON cst.branch_id = bm.Branch_id AND bm.Customer_ID != 0 WHERE bm.Deleted_Flag = 0 AND bm.site_type = ${admin.sitetype} AND 
      bm.Customer_id = ${admin.companyid} GROUP BY bm.Branch_id, bm.Customer_id, bm.contact_person, bm.Address, bm.city, bm.state, bm.pincode, bm.site_starttime, bm.site_endtime, 
      bm.Email_id, bm.Contact_no, bm.whatsappgroupname, bm.Image_hotlinepath, bm.Image_archivalpath, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, bm.Branch_name,
      bm.status, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, bm.branch_code, bm.gstno, bm.hidestatus ORDER BY bm.Branch_id;
      `);
      ...
      sql =
       ...
       bm.sitecontroller_status = 1 THEN DATE_FORMAT(bm.site_uptime, '%Y-%m-%d %H:%i:%s') ELSE DATE_FORMAT(bm.site_downtime, '%Y-%m-%d %H:%i:%s') END AS time, bm.Branch_name,
       bm.status AS Sitestatus, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, COUNT(dm.Device_id) AS Total_Devices, bm.branch_code, bm.gstno, SUM(CASE WHEN dm.runningstatus = 1 
       THEN 1 ELSE 0 END) AS Active_Devices, SUM(CASE WHEN dm.runningstatus = 0 THEN 1 ELSE 0 END) AS Inactive_Devices, bm.hidestatus,CASE WHEN cst.branch_id IS NOT NULL THEN 1 ELSE 0 END AS subscription,bm.Branch_Logo FROM branchmaster bm LEFT JOIN deptmaster dt ON bm.Branch_id
       = dt.Branch_id LEFT JOIN devicemaster dm ON dt.Dept_id = dm.Dept_id AND dm.Deleted_Flag = 0 LEFT JOIN subscriptioncustomertrans cst ON cst.branch_id = bm.Branch_id AND bm.Customer_ID != 0 WHERE bm.Deleted_Flag = 0  AND bm.status = 1 AND bm.site_type = ${admin.sitetype} AND
       bm.Customer_id IN (${admin.companyid}) GROUP BY bm.Branch_id, bm.Customer_id, bm.contact_person, bm.Address, bm.city, bm.state, bm.pincode, bm.site_starttime, bm.site_endtime,
       bm.Email_id, bm.Contact_no, bm.whatsappgroupname, bm.Image_hotlinepath, bm.Image_archivalpath, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, bm.Branch_name,
       bm.status, bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, bm.branch_code, bm.gstno, bm.hidestatus;`);
    } else if (
      ...
      sql =
       ...
       bm.whatsappgroupname, bm.Image_hotlinepath AS hotline, bm.Image_archivalpath AS Archival, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, CASE WHEN bm.sitecontroller_status 
       = 1 THEN DATE_FORMAT(bm.site_uptime, '%Y-%m-%d %H:%i:%s') ELSE DATE_FORMAT(bm.site_downtime, '%Y-%m-%d %H:%i:%s') END AS time, bm.Branch_name, bm.status AS Sitestatus,
       bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, COUNT(dm.Device_id) AS Total_Devices, bm.branch_code, bm.gstno, SUM(CASE WHEN dm.runningstatus = 1 THEN 1 ELSE 0 END) AS
       Active_Devices, SUM(CASE WHEN dm.runningstatus = 0 THEN 1 ELSE 0 END) AS Inactive_Devices, bm.hidestatus,CASE WHEN cst.branch_id IS NOT NULL THEN 1 ELSE 0 END AS subscription,bm.Branch_Logo FROM branchmaster bm LEFT JOIN deptmaster dt ON bm.Branch_id = dt.Branch_id LEFT JOIN 
       devicemaster dm ON dt.Dept_id = dm.Dept_id AND dm.Deleted_Flag = 0 LEFT JOIN subscriptioncustomertrans cst ON cst.branch_id = bm.Branch_id AND bm.Customer_ID != 0 WHERE bm.Deleted_Flag = 0 AND bm.status = 0 AND bm.site_type = ${admin.sitetype} AND bm.Customer_id IN
       (${admin.companyid}) GROUP BY bm.Branch_id, bm.Customer_id, bm.contact_person, bm.Address, bm.city, bm.state, bm.pincode, bm.site_starttime, bm.site_endtime, bm.Email_id, 
       bm.Contact_no, bm.whatsappgroupname, bm.Image_hotlinepath, bm.Image_archivalpath, bm.Image_backupdays, bm.snapcount, bm.Sitecontroller_path, bm.Branch_name, bm.status, 
       bm.SiteController_Status, bm.Site_uptime, bm.Site_Downtime, bm.branch_code, bm.gstno, bm.hidestatus;
      `);
...
async function getsitepath(admin) {
  try {
    const sql = await db.query(
      `select branch_id,Sitecontroller_path from branchmaster;`
    );
    const code = true;
    const message = "Site list Fetched Successfully";
    return { code, message, sql };
  } catch (er) {
    const code = false;
    const message = "Internel error. Please try again later.";
...
async function getdevicelist(admin) {
  try {
   ...
   Device_type,CASE WHEN dm.SDK_ID = 1 THEN 'Dahua' WHEN dm.SDK_ID = 2 THEN 'Hikvision' WHEN dm.SDK_ID = 3 THEN 'CP plus' WHEN dm.SDK_ID = 4 THEN 'TVT' WHEN dm.SDK_ID = 5 THEN 'UNV'
   ELSE 'Unknown' END AS Brand_Name,dm.IP_Domain,dm.secondary_ip secondaryip,dm.IP_Port,dm.IP_Uname,dm.IP_Pwd,
      ...
      switch (admin.status) {
        ...
        case "deactivate":
          ...
          if (result[0].Branch_name) {
            const sitename = result[0].Branch_name;
            // const url = helper.hashSiteId(siteId, config.sitesalt);
            EmailSent = await mailer.sendEmail(
              "Administrator",
              "ganeshkumar.m@sporadasecure.com,ramachandran.m@sporadasecure.com,support@sporadasecure.com,spandi.a@sporadasecure.com,jayakrishna.k@sporadasecure.com,hariprasath.sporadasecure.com,arunkumar.m@sporadasecure.com",
              // ,ceo@sporadasecure.com.ramachadran.m@sporadasecure.com',
              `Action Required!!! Received Site Deactivation Request for ${sitename}`,
              "deletesite.html",
              "http://api.ssipl.org/admin/deactivate?siteid=" + siteId,
              "DEACTIVATE_SITE",
              sitename,
              "SITE DEACTIVATION"
            );
            ...
            if (EmailSent == true) {
              responses.push({
                siteid: siteId,
                message: `An email has been sent to the administrator regarding the deactivation request for Site ID: ${siteId}. You will be notified once the request is processed.`,
              });
            } else {
              responses.push({
                siteid: siteId,
                message: `There was an issue sending the deactivation request email for Site ID: ${siteId}. Please try again or contact support if the issue persists.`,
              });
            }
          }
          break;
        case "delete":
          ...
          if (result1[0].Branch_name) {
            const sitename = result1[0].Branch_name;
            // const url = helper.hashSiteId(siteId, config.sitesalt);
            EmailSent = await mailer.sendEmail(
              "Administrator",
              "ganeshkumar.m@sporadasecure.com,ramachandran.m@sporadasecure.com,support@sporadasecure.com,spandi.a@sporadasecure.com,jayakrishna.k@sporadasecure.com,hariprasath.sporadasecure.com,arunkumar.m@sporadasecure.com",
              // ,ceo@sporadasecure.com.ramachadran.m@sporadasecure.com',
              `Action Required!!! Received Site Deletion Request for ${sitename}`,
              "acverification.html",
              "http://api.ssipl.org/admin/sdelete?siteid=" + siteId,
              "DELETE_SITE",
              sitename,
              "SITE DELETION"
            );
            ...
            if (EmailSent == true) {
              responses.push({
                siteid: siteId,
                message: `An email has been sent to the administrator regarding the deletion request for Site ID: ${siteId}. You will be notified once the request is processed.`,
              });
            } else {
              responses.push({
                siteid: siteId,
                message: `There was an issue sending the deletion request email for Site ID: ${siteId}. Please try again or contact support if the issue persists.`,
              });
            }
          }
          break;
        case "tolive":
          dbstatus = 4;
          const sql = await db.query(
            `Update Branchmaster set Site_type = 0 where branch_id = ?`,
            [siteId]
          );
          responses.push({
            siteid: siteId,
            message: `Site ${siteId} Modified successfully.`,
          });
          break;
        default:
          return helper.getErrorResponse(
      ...
      try {
        if (dbstatus == 1) {
          ...
          if (status !== undefined && status !== null && status !== 0) {
            ...
            responses.push({
              siteid: siteId,
              message: `Error while modifying site ${siteId}`,
            });
          }
        } else {
        }
...
async function UpdateSite(admin) {
  try {
    ...
    const requiredFields = [
      { key: "companyid", message: "Company ID is missing." },
      { key: "sitename", message: "Site name is missing." },
      { key: "address", message: "Site address is missing." },
      { key: "city", message: "City for the site is missing." },
      { key: "state", message: "State for the site is missing." },
      { key: "pincode", message: "PIN code for the site is missing." },
      { key: "hotlinestorage", message: "Hotline storage path is missing." },
      { key: "archivalpath", message: "Archival storage path is missing." },
      { key: "backupdays", message: "Image backup days are missing." },
      {
        key: "sitecontrollerpath",
        message: "Site Controller path is missing.",
      },
      { key: "snapcount", message: "Snap count is missing." },
      { key: "starttime", message: "Start time is missing." },
      { key: "endtime", message: "End time is missing." },
      { key: "siteid", message: "Site ID is missing." },
      { key: "whatsappgroupname", message: "Whatsapp group name missing." },
    ];

    // Validate required fields
    for (const field of requiredFields) {
      if (!admin.hasOwnProperty(field.key)) {
        return helper.getErrorResponse(
          false,
          `${field.message} Please provide a valid ${field.key}.`,
          "CUSTOMER UPDATE SITE",
          ""
        );
      }
    ...
    // Handle optional fields with specific responses
    const optionalResponses = [];
    if (!admin.branchcode) {
      optionalResponses.push({
        code: false,
    ...
    // Sanitize sitename
    const sanitizedSitename = admin.sitename
      .replace(/\s/g, "")
      .replace(/[^\w\s]/gi, "");
    const siteFolderPath = path.join(
      admin.sitecontrollerpath,
      sanitizedSitename
    );
    ...
    const status = result[1][0]["@result"];
    console.log(`Result branch ID => ${status}`);

    if (status !== 1) {
      return helper.getErrorResponse(
        false,
        "Customer site update failed. Please try again.",
        "error",
        ""
      );
    }
    try {
      // Create folder if the site update was successful
      const folderPath = await createFolder(
        admin.sitecontrollerpath,
        sanitizedSitename
      );
      console.log(`Event folder path -> ${folderPath}`);
      ...
      // Add site config
      await addDepartment(admin.siteid, admin.sitename);
      await addsiteconfig(admin.siteid);
...
async function OrganizationList(admin) {
  try {
    ...
    if (admin.sitetype == 0) {
      data = await db.query(
        `SELECT Organization_id,Email_id,Organization_Name,orgcode, Organization_Logo,Address from organizations where status = 1 and deleted_flag =0 and Organization_type =2  and Site_type =0`
      );
    } else if (admin.sitetype == 1) {
      data = await db.query(
        `SELECT Organization_id,Email_id,Organization_Name,orgcode, Organization_Logo,Address from organizations where status = 1 and deleted_flag =0 and Organization_type =2  and Site_type =1`
      );
    }
  ...
  try {
    ...
    if (admin.sitetype == 0) {
      if (
        ...
        data = await db.query(
          `SELECT Customer_id,Organization_id,Customer_name,ccode,Email_id,Customer_Logo,address,billing_address from customermaster where site_type = 0 and status = 1 and deleted_flag =0 and Customer_type In(1,2) and Organization_id = ${admin.organizationid};`
        );
      } else {
        data = await db.query(
          `SELECT Customer_id,Organization_id,Customer_name,ccode,Email_id,Customer_Logo,address,billing_address from customermaster where site_type = 0 and status = 1 and deleted_flag =0 and Customer_type = 1 and Organization_id = 0;`
        );
      }
    } else if (admin.sitetype == 1) {
      if (
        ...
        data = await db.query(
          `SELECT Customer_id,Organization_id,Customer_name,ccode,Email_id,Customer_Logo,,address,billing_address from customermaster where site_type = 1 and status = 1 and deleted_flag =0 and Customer_type In(1,2) and Organization_id = ${admin.organizationid};`
        );
      } else {
        data = await db.query(
          `SELECT Customer_id,Organization_id,Customer_name,ccode,Email_id,Customer_Logo,address,billing_address from customermaster where site_type = 1 and status = 1 and deleted_flag =0 and Customer_type = 1 and Organization_id = 0;`
        );
      }
    } else {
      data = await db.query(
        `SELECT Customer_id,Organization_id,Customer_name,ccode,Email_id,Customer_Logo,address,billing_address from customermaster where status = 1 and deleted_flag =0`
      );
    }
    const code = true;
...
async function Individuallist(admin) {
  try {
    ...
    if (admin.sitetype == 0) {
      data = await db.query(
        `SELECT Customer_id,Customer_name,ccode,Email_id,Customer_Logo from customermaster where Site_type = 0 and status = 1 and deleted_flag =0 and Customer_type = 3 and Organization_id = 0;`
      );
    } else {
      data = await db.query(
...
async function DemoList(admin) {
  try {
    ...
    data = await db.query(
      `SELECT Customer_id,Customer_name,ccode,Email_id,Customer_Logo from customermaster where Site_type = 1 and status = 1 and deleted_flag =0 and Customer_type in(1,2,3);`
    );
    const code = true;
    ...
    for (const admin of organization) {
      ...
      if (!admin.hasOwnProperty("emailid")) {
        results.push({
          code: false,
          message:
            "Organization Email ID is missing. Please provide the Email ID.",
          name: organizationname,
        });
        continue;
      }
      ...
      if (!admin.hasOwnProperty("contactno")) {
        results.push({
          code: false,
          message:
            "Organization contact number is missing. Please provide the contact number.",
          name: organizationname,
        });
        continue;
      }
      ...
      if (!admin.hasOwnProperty("address")) {
        results.push({
          code: false,
          message:
            "Organization address is missing. Please provide the address.",
          name: organizationname,
        });
        continue;
      }
      ...
      if (!admin.hasOwnProperty("contactperson")) {
        results.push({
          code: false,
          message:
            "Organization contact person is missing. Please provide the contact person.",
          name: organizationname,
        });
        continue;
      }
...
async function getDeviceDetails(admin) {
  try {
    ...
    const sql = await db.query(
      `SELECT SerialNo ,Dept_ID,Device_name,SDK_ID, Product_name,secondary_ip secondaryip,IP_Domain,IP_Port,IP_Uname,IP_Pwd,No_AnalogCH,No_IpCH,	MotionDetection,httpport,RTSP_Port,Model_no,Harddisk,harddiskcap,	harddiskfreespace,software_version from devicemaster where device_id in(${admin.deviceid});`
    );
...
async function AddAIMachine(admin) {
  try {
    ...
    if (admin.hasOwnProperty("configpath") == false) {
      return helper.getErrorResponse(
        false,
        "Config path missing. Please provide the config path.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("enginepath") == false) {
      return helper.getErrorResponse(
        false,
        "Engine path missing. Please provide the Engine path.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("preprocess") == false) {
      return helper.getErrorResponse(
        false,
        "Pre processor address missing. Please provide the Pre Processor.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("postprocess") == false) {
      return helper.getErrorResponse(
        false,
        "Post processor address missing. Please provide the post Processor.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("lcpostprocess") == false) {
      return helper.getErrorResponse(
        false,
        "Low confidence Post process machine missing. Please provide the Low confidence post process.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("interval") == false) {
      return helper.getErrorResponse(
        false,
        "AI interval missing. Please provide the AI interval.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("cudadevice") == false) {
      return helper.getErrorResponse(
        false,
        "Cuda device missing. Please provide the cuda device.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("confidence") == false) {
      return helper.getErrorResponse(
        false,
        "Confidence missing. Please provide the confidence.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("websocketip") == false) {
      return helper.getErrorResponse(
        false,
        "Web socket ip missing. Please provide the websocket ip.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("websocketport") == false) {
      return helper.getErrorResponse(
        false,
        "Web socket port missing. Please provide the websocket port.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("startbatpath") == false) {
      return helper.getErrorResponse(
        false,
        "Start bat path missing. Please provide the start bat path.",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    if (admin.hasOwnProperty("stopbatpath") == false) {
      return helper.getErrorResponse(
        false,
        "Stop bat path missing. Please provide the stop bat path",
        "ADD AI SERVER MACHINE",
        ""
      );
    }
    ...
    try {
      ...
      if (aiid != null && aiid != 0) {
        await fs.mkdir(admin.configpath, { recursive: true });
        const sourceFolder = config.aienginepath;
        const files = await fs.readdir(sourceFolder, { withFileTypes: true });
        const engineFile = files.find(
          (file) => file.isFile() && file.name.endsWith(".engine")
        );
        if (!engineFile) {
          return helper.getErrorResponse(
            false,
            "Engine file not available in the source path",
            "ADD AI SERVER",
            ""
          );
        }
        const engineName = engineFile.name;
        var aiid1 = aiid.toString();
        const enginepath = path.join(admin.configpath, aiid1, engineName);
        const filename = "config.json";
        const filepathai = path.join(admin.configpath, aiid1);
        await cpy.copy(sourceFolder, filepathai);
        const filepath = path.join(filepathai, filename);
        console.log(`combined patt ` + filepath);

        try {
          const jsonArray = {
            machine_id: aiid,
            cuda_device: admin.cudadevice,
            engine_path: enginepath,
            config_path: filepath,
            pre_process_address: admin.preprocess,
            post_process_address: admin.postprocess,
            confidence_threshold: admin.confidence,
            lowconfidence_post_process_address: admin.lcpostprocess,
            interval: admin.interval,
            websocket_ip: admin.websocketip,
            websocket_port: admin.websocketport || 0,
          };
          // Write JSON data to file
          const fileHandle = await fs.open(filepath, "w");
          await fileHandle.writeFile(JSON.stringify(jsonArray, null, 4));
          await fileHandle.close();
          const sql1 = await db.query(
            `Update aisysmaster set new_enginepath = ?,config_path = ? where machine_id = ?`,
            [enginepath, filepath, aiid]
          );
          if (sql1.affectedRows)
            return helper.getSuccessResponse(
              true,
              "AI Inference added successfully",
              aiid,
              ""
            );
        } catch (er) {
...
async function AddPreProcessor(admin) {
  try {
    if (admin.hasOwnProperty("preprocessorname") == false) {
      return helper.getErrorResponse(
        false,
        "Pre processor name missing. Please provide the Pre processor name",
        "ADD PRE PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("preprocessorip") == false) {
      return helper.getErrorResponse(
        false,
        "Pre processor ip address missing. Please provide the Pre processor IP address.",
        "ADD PRE PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("preprocessorpath") == false) {
      return helper.getErrorResponse(
        false,
        "Pre processor path missing. Please provide the Pre processor path.",
        "ADD PRE PROCESSOR SERVER",
        ""
      );
    }
    ...
    if (admin.hasOwnProperty("redisip") == false) {
      return helper.getErrorResponse(
        false,
        "Redis ip missing. Please provide the Redis ip.",
        "ADD PRE PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("redisport") == false) {
      return helper.getErrorResponse(
        false,
        "Redis port missing. Please provide the Redis port.",
        "ADD PRE PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("rediskey") == false) {
      return helper.getErrorResponse(
        false,
        "Redis key missing. Please provide the Redis key.",
        "ADD PRE PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("maxworkers") == false) {
      return helper.getErrorResponse(
        false,
        "Max Workers missing. Please provide the Max Workers.",
        "ADD PRE PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("grpcaddress") == false) {
      return helper.getErrorResponse(
        false,
        "Grpc address missing. Please provide the Grpc address.",
        "ADD PRE PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("batchsize") == false) {
      return helper.getErrorResponse(
        false,
        "Batch size missing. Please provide the batch size",
        "ADD PRE PROCESSOR SERVER",
        ""
      );
    }
    ...
    try {
      const [result] = await db.spcall(
        `CALL SP_ADD_PREPROCESSOR(?,?,?,?,?,?,?,?,?,?,?,?,@aiid); select @aiid;`,
        [
          admin.preprocessorname,
          admin.preprocessorip,
          admin.preprocessorpath,
          admin.redisip,
          admin.redisport,
          admin.rediskey,
          admin.startbatpath,
      ...
      const objectvalue = result[1][0];
      const preprocessorid = objectvalue["@aiid"];
      console.log(`event ai id -> ${preprocessorid}`);
      if (preprocessorid != null && preprocessorid != 0) {
        var preprocessorid1 = preprocessorid.toString();
        const confgpath = path.join(admin.configpath, preprocessorid1);
        await fs.mkdir(confgpath, { recursive: true });
        const filename = "config.json";
        // const filepathai = path.join(admin.configpath, preprocessorid1);
        const filepath = path.join(confgpath, filename);
        console.log(`combined patt ` + filepath);

        try {
          const jsonArray = {
            aipath_host: admin.redisip,
            db_port: admin.redisport,
            batch_size: admin.batchsize,
            redis_key: admin.rediskey,
            max_workers: admin.maxworkers,
            pre_process_address: admin.grpcaddress,
          };
          // Write JSON data to file
          const fileHandle = await fs.open(filepath, "w");
          await fileHandle.writeFile(JSON.stringify(jsonArray, null, 4));
          await fileHandle.close();
          const sql1 = await db.query(
            `Update aisysmaster set config_path = ? where machine_id = ?`,
            [filepath, preprocessorid]
          );
          if (sql1.affectedRows)
            return helper.getSuccessResponse(
              true,
              "Preprocessor added successfully",
              preprocessorid,
              ""
            );
        } catch (er) {
...
async function AddPostProcessor(admin) {
  try {
    if (admin.hasOwnProperty("postprocessorname") == false) {
      return helper.getErrorResponse(
        false,
        "post processor name missing. Please provide the Pre processor name",
        "ADD POST PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("postprocessorip") == false) {
      return helper.getErrorResponse(
        false,
        "Post processor ip address missing. Please provide the Pre processor IP address.",
        "ADD POST PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("postprocessorpath") == false) {
      return helper.getErrorResponse(
        false,
        "Post processor path missing. Please provide the Pre processor path.",
        "ADD POST PROCESSOR SERVER",
        ""
      );
    }
    ...
    if (admin.hasOwnProperty("machinetype") == false) {
      return helper.getErrorResponse(
        false,
        "Machine type missing. Please provide the Machine type",
        "ADD POST PROCESSOR SERVER",
        ""
      );
    }

    if (admin.machinetype == 1) {
      admin.machinetype = 3;
    } else if (admin.machinetype == 2) {
      admin.machinetype = 4;
    }
    try {
      const [result] = await db.spcall(
        `CALL SP_ADD_POSTPROCESSOR(?,?,?,?,?,?,?,?,?,?,?,?,@aiid); select @aiid;`,
        [
          admin.postprocessorname,
          admin.postprocessorip,
          admin.postprocessorpath,
          admin.redisip,
          admin.redisport,
          admin.rediskey,
          admin.startbatpath,
      ...
      const objectvalue = result[1][0];
      const postprocessorid = objectvalue["@aiid"];
      console.log(`event ai id -> ${postprocessorid}`);
      if (postprocessorid != null && postprocessorid != 0) {
        var postprocessorid1 = postprocessorid.toString();
        const confgpath = path.join(admin.configpath, postprocessorid1);
        await fs.mkdir(confgpath, { recursive: true });
        const filename = "config.json";
        // const filepathai = path.join(admin.configpath, preprocessorid1);
        const filepath = path.join(confgpath, filename);
        ...
        try {
          var jsonArray;
          if (admin.machinetype == 3) {
            jsonArray = {
              max_workers: admin.maxworkers,
              post_address: admin.grpcaddress,
              aipath_host: admin.redisip,
              aipath_port: admin.redisport,
              aipath_keyname: admin.rediskey,
            };
          } else {
            jsonArray = {
              max_workers: admin.maxworkers,
              lowconffidence_post_address: admin.grpcaddress,
            };
          }
          // Write JSON data to file
          const fileHandle = await fs.open(filepath, "w");
          ...
          const sql1 = await db.query(
            `Update aisysmaster set config_path = ? where machine_id = ?`,
            [filepath, postprocessorid]
          );
          if (sql1.affectedRows)
            return helper.getSuccessResponse(
              true,
              "Post processor added successfully",
              postprocessorid,
              ""
            );
        } catch (er) {
...
async function GetAImachine(admin) {
  try {
    ...
    if (admin?.machinetype == 2) {
      if (admin.hasOwnProperty("aimachineid") == true) {
        sql =
          await db.query(`select DISTINCT Machine_id aimachineid,Machine_name aimachinename,MachineIP AImachineip,Preprocess_server preprocess,Postprocess_server postprocess,Low_confidence_server lcpostprocess,
          Config_path configpath,Config_folder configfolder, new_enginepath enginepath,AIInterval 'interval',cuda_device cudadevice,Confidence_thershold confidence,websocketip,websocketport, Bat_filepath startbatpath,Stop_batpath stopbatpath,aimachine_path aimachinepath,Process_id processid,error_log,error_time,
          status,runningstatus running_status from aisysmaster where Machine_id = ${admin.aimachineid} and deleted_flag = 0 and Machine_type = ${admin.machinetype} ORDER BY Machine_id;`);
      } else {
        sql =
          await db.query(`select DISTINCT Machine_id aimachineid,Machine_name aimachinename,MachineIP AImachineip,Preprocess_server preprocess,Postprocess_server postprocess,Low_confidence_server lcpostprocess,
          Config_path configpath,Config_folder configfolder, new_enginepath enginepath,AIInterval 'interval',cuda_device cudadevice,Confidence_thershold confidence,websocketip,websocketport, Bat_filepath startbatpath,Stop_batpath stopbatpath,aimachine_path aimachinepath,Process_id processid,error_log,error_time,
          status,runningstatus running_status from aisysmaster where deleted_flag = 0 and Machine_type = ${admin.machinetype} ORDER BY Machine_id;`);
      }
    } else if (admin?.machinetype == 1) {
      if (admin.hasOwnProperty("aimachineid") == true) {
        sql =
          await db.query(`select DISTINCT Machine_id preprocessorid,Machine_name preprocessorname,MachineIP preprocessorip,redis_ip redisip,redis_port redisport,Redis_key rediskey,max_workers maxworkers,
        grpc_address_port grpcaddress,Batch_size batchsize,Config_path configpath,Config_folder configfolder,Bat_filepath startbatpath,Stop_batpath stopbatpath,aimachine_path preprocessorpath,Process_id processid,error_log,error_time,status,
      runningstatus running_status from aisysmaster where Machine_id = ${admin.aimachineid} and deleted_flag = 0 and Machine_type = ${admin.machinetype} ORDER BY Machine_id;`);
      } else {
        sql =
          await db.query(`select DISTINCT Machine_id preprocessorid,Machine_name preprocessorname,MachineIP preprocessorip,redis_ip redisip,redis_port redisport,Redis_key rediskey,max_workers maxworkers,
        grpc_address_port grpcaddress,Batch_size batchsize,Config_path configpath,Config_folder configfolder,Bat_filepath startbatpath,Stop_batpath stopbatpath,aimachine_path preprocessorpath,Process_id processid,error_log,error_time,status,
      runningstatus running_status from aisysmaster where deleted_flag = 0 and Machine_type = ${admin.machinetype} ORDER BY Machine_id;`);
      }
    } else if (admin?.machinetype == 3 || admin?.machinetype == 4) {
      if (admin.hasOwnProperty("aimachineid") == true) {
        sql =
          await db.query(`select DISTINCT Machine_id postprocessorid,Machine_name postprocessorname,MachineIP postprocessorip,redis_ip redisip,redis_port redisport,Redis_key rediskey,max_workers maxworkers,
        grpc_address_port grpcaddress,Config_path configpath,Config_folder configfolder,Bat_filepath startbatpath,Stop_batpath stopbatpath,aimachine_path postprocessorpath,Process_id processid,error_log,error_time,status,
      runningstatus running_status from aisysmaster where Machine_id = ${admin.aimachineid} and deleted_flag = 0 and Machine_type = ${admin.machinetype} ORDER BY Machine_id;`);
      } else {
        sql =
          await db.query(`select DISTINCT Machine_id postprocessorid,Machine_name postprocessorname,MachineIP postprocessorip,redis_ip redisip,redis_port redisport,Redis_key rediskey,max_workers maxworkers,
      ...
      return {
        code: true,
        message: "Please provide the valid machine type",
        error: "Incorrect machine type",
      };
    }
    return {
      code: true,
...
async function GetAImachineProcess(admin) {
  try {
    var sql =
      await db.query(`select DISTINCT Machine_id , Bat_filepath startbatpath,Stop_batpath stopbatpath,aimachine_path aimachinepath,Process_id processid,error_log,error_time,
          status,runningstatus running_status from aisysmaster where deleted_flag = 0 and status = 1 ORDER BY Machine_id;`);
    if (sql[0]) {
      return {
        code: true,
        message: "Available AI machine fetched successfully.",
      ...
      return {
        code: false,
        message: "There is no AI machine available",
        error: "GET AI MACHINE PROCESS ID FAILED",
      };
    }
  } catch (er) {
    return {
...
//       );
//     }
//     if (admin.hasOwnProperty("configfolder") == false) {
//       return helper.getErrorResponse(
//         false,
//         "Config folder missing. Please provide the config folder.",
//         "UPDATE AI SERVER MACHINE",
//         ""
//       );
//     }
...
//     try {
//       if (admin.aimachineid != null && admin.aimachineid != "") {
//         await fs.mkdir(admin.configfolder, { recursive: true });
//         const sourceFolder = config.aienginepath;
//         const files = await fs.readdir(sourceFolder, { withFileTypes: true });
//         const engineFile = files.find(
//           (file) => file.isFile() && file.name.endsWith(".engine")
...
//         }
//         const engineName = engineFile.name;
//         var aiid1 = admin.aimachineid.toString();
//         const enginepath = path.join(admin.configfolder, aiid1, engineName);
//         const filename = "config.json";
//         const filepathai = path.join(admin.configfolder, aiid1);
//         await cpy.copy(sourceFolder, filepathai);
//         const filepath = path.join(filepathai, filename);

//         try {
//           const jsonArray = {
//             machine_id: admin.aimachineid,
//             cuda_device: admin.cudadevice,
//             engine_path: enginepath,
//             pre_process_address: admin.preprocess,
//             post_process_address: admin.postprocess,
...
//           await fileHandle.writeFile(JSON.stringify(jsonArray, null, 4));
//           await fileHandle.close();
//           const query = `UPDATE aisysmaster SET Machine_name = ?, MachineIP = ?, aimachine_path = ?, new_enginepath = ?, Bat_filepath = ? , Stop_batpath = ?, cuda_device = ?, AIInterval = ?,
//       Confidence_thershold = ? , Preprocess_server = ?, Postprocess_server = ?, Low_confidence_server = ?, Config_path = ?,config_folder = ? , websocketip = ?, websocketport = ? WHERE Machine_id = ?;`;
...
//             return helper.getSuccessResponse(
//               true,
//               "AI server Updated successfully",
//               admin.aimachineid,
//               ""
//             );
//           } else {
  ...
  try {
    ...
    for (const field of requiredFields) {
      if (!admin.hasOwnProperty(field)) {
        return helper.getErrorResponse(
          false,
          `${field} missing. Please provide the ${field}.`,
          "UPDATE AI SERVER MACHINE",
          ""
        );
      }
    ...
    const enginepath = path.join(
      admin.configfolder,
      admin.aimachineid.toString(),
      engineName
    );
    const filepathai = path.join(
      admin.configfolder,
      admin.aimachineid.toString()
    );
    const filepath = path.join(filepathai, "config.json");
    ...
    await fs.writeFile(filepath, JSON.stringify(configData, null, 4), "utf-8");
    ...
    if (result.affectedRows) {
      ...
      return helper.getErrorResponse(
        false,
        "Error updating the AI Inference.",
        "",
        ""
      );
    }
  } catch (error) {
    console.error("Error in AI server update:", error);
    return helper.getErrorResponse(
      false,
      "Internal error. Please contact Administration",
      error.message,
...
async function UpdatePreProcessor(admin) {
  try {
    if (admin.hasOwnProperty("preprocessorname") == false) {
      return helper.getErrorResponse(
        false,
        "Pre processor name missing. Please provide the Pre processor name",
        "UPDATE PRE PROCESSOR SERVER",
        ""
      );
    }
    if (admin.hasOwnProperty("preprocessorip") == false) {
    ...
    if (admin.hasOwnProperty("preprocessorid") == false) {
      return helper.getErrorResponse(
        false,
        "Pre processor id missing. Please provide the Pre processor id",
        "UPDATE PRE PROCESSOR SERVER",
        ""
      );
    }
    try {
      if (admin.preprocessorid != null && admin.preprocessorid != 0) {
        var preprocessorid1 = admin.preprocessorid.toString();
        const confgpath = path.join(admin.configfolder, preprocessorid1);
        await fs.mkdir(confgpath, { recursive: true });
        const filename = "config.json";
        // const filepathai = path.join(admin.configpath, preprocessorid1);
        const filepath = path.join(confgpath, filename);
        ...
        try {
          ...
          await fileHandle.writeFile(JSON.stringify(jsonArray, null, 4));
          await fileHandle.close();
          const query = `UPDATE aisysmaster SET Machine_name = ?, MachineIP = ?, aimachine_path = ?,Redis_ip = ?,Redis_port = ?,Redis_key = ?, Bat_filepath = ? , Stop_batpath = ?,
            max_workers = ?, grpc_address_port = ?, batch_size = ?, Config_path = ? , Config_folder =? WHERE Machine_id = ?;`;
          ...
          if (result.affectedRows) {
            ...
            return helper.getErrorResponse(
              false,
              "Error Updated the Preprocessor.",
              "UPDATE PRE PROCESSOR SERVER",
              ""
            );
          }
...
// async function UpdatePostProcessor(admin) {
//   try{
//     if (admin.hasOwnProperty("postprocessorname") == false) {
//       return helper.getErrorResponse(false,"Pre processor name missing. Please provide the Pre processor name","ADD POST PROCESSOR SERVER","");
//     }
//     if (admin.hasOwnProperty("postprocessorip") == false) {
//       return helper.getErrorResponse(false,"Pre processor ip address missing. Please provide the Pre processor IP address.","ADD POST PROCESSOR SERVER","");
//     }
//     if (admin.hasOwnProperty("postprocessorpath") == false) {
//       return helper.getErrorResponse(false,"Pre processor path missing. Please provide the Pre processor path.","ADD POST PROCESSOR SERVER","");
//     }
//     if (admin.hasOwnProperty("configpath") == false) {
//       return helper.getErrorResponse(false,"Config path missing. Please provide the config path.","ADD POST PROCESSOR SERVER","");
//     }
//     if (admin.hasOwnProperty("redisip") == false) {
//       return helper.getErrorResponse(false,"Redis ip missing. Please provide the Redis ip.","ADD POST PROCESSOR SERVER","");
//     }
//     if (admin.hasOwnProperty("redisport") == false) {
//       return helper.getErrorResponse(false,"Redis port missing. Please provide the Redis port.","ADD POST PROCESSOR SERVER","");
//     }
//     if (admin.hasOwnProperty("rediskey") == false) {
//       return helper.getErrorResponse(false,"Redis key missing. Please provide the Redis key.","ADD POST PROCESSOR SERVER","");
//     }
//     if (admin.hasOwnProperty("maxworkers") == false) {
//       return helper.getErrorResponse(false,"Max Workers missing. Please provide the Max Workers.","ADD POST PROCESSOR SERVER","");
//     }
//     if (admin.hasOwnProperty("grpcaddress") == false) {
//       return helper.getErrorResponse(false,"Grpc address missing. Please provide the Grpc address.","ADD POST PROCESSOR SERVER","");
//     }
//     if(admin.hasOwnProperty('startbatpath') == false){
//       return helper.getErrorResponse(false,'Start bat path missing. Please provide the start bat path','ADD POST PROCESSOR SERVER',"");
//     }
//     if(admin.hasOwnProperty('stopbatpath') == false){
//       return helper.getErrorResponse(false,'Stop bat path missing. Please provide the stop bat path','ADD POST PROCESSOR SERVER',"");
//     }
//     if(admin.hasOwnProperty('machinetype') == false){
//       return helper.getErrorResponse(false,'Machine type missing. Please provide the Machine type',"ADD POST PROCESSOR SERVER","");
//     }
//     if(admin.hasOwnProperty('postprocessorid') == false){
//       return helper.getErrorResponse(false,'Post processor id missing. Please provide the Post processor id','UPDATE POST PROCESSOR SERVER',"");
//     }
//     try {
//       if(admin.machinetype == 1){
//         admin.machinetype = 3;
...
//       }
//       try {
//         if (admin.postprocessorid != null && admin.postprocessorid != 0) {
//           var postprocessorid1 = admin.postprocessorid.toString();
//             const confgpath = path.join(admin.configpath,postprocessorid1);
//             await fs.mkdir(confgpath, { recursive: true });
//             const filename = "config.json";
//             // const filepathai = path.join(admin.configpath, preprocessorid1);
...
//             await fileHandle.close();
//             const query = `UPDATE aisysmaster SET Machine_name = ?, MachineIP = ?, aimachine_path = ?,Redis_ip = ?,Redis_port = ?,Redis_key = ?, Bat_filepath = ? , Stop_batpath = ?,
//             max_workers = ?, grpc_address_port = ?, Config_path = ?,Machine_type = ?  WHERE Machine_id = ?;`;
...
async function UpdatePostProcessor(admin) {
  try {
    // Validate required fields
    const requiredFields = [
      { key: "postprocessorname", message: "Post processor name missing" },
      { key: "postprocessorip", message: "Post processor IP address missing" },
      { key: "postprocessorpath", message: "Post processor path missing" },
      { key: "configpath", message: "Config path missing" },
      { key: "configfolder", message: "Config folder missing" },
      { key: "redisip", message: "Redis IP missing" },
      { key: "redisport", message: "Redis port missing" },
      { key: "rediskey", message: "Redis key missing" },
      { key: "maxworkers", message: "Max Workers missing" },
      { key: "grpcaddress", message: "gRPC address missing" },
      { key: "startbatpath", message: "Start bat path missing" },
      { key: "stopbatpath", message: "Stop bat path missing" },
      { key: "machinetype", message: "Machine type missing" },
      { key: "postprocessorid", message: "Post processor ID missing" },
    ];
    ...
    // Prepare configuration path and file
    const postProcessorIdStr = admin.postprocessorid.toString();
    const configPath = path.join(admin.configfolder, postProcessorIdStr);
    ...
    const jsonArray =
      admin.machinetype === 3
        ...
        : {
            max_workers: admin.maxworkers,
            lowconfidence_post_address: admin.grpcaddress,
          };

    // Write JSON config file
    await fs.writeFile(filePath, JSON.stringify(jsonArray, null, 4));

    // Update database
    const query = `
      UPDATE aisysmaster 
      SET Machine_name = ?, MachineIP = ?, aimachine_path = ?, Redis_ip = ?, Redis_port = ?, Redis_key = ?, 
          Bat_filepath = ?, Stop_batpath = ?, max_workers = ?, grpc_address_port = ?, Config_path = ? ,config_folder= ?
      WHERE Machine_id = ?;
    `;
    const values = [
      admin.postprocessorname,
    ...
    if (result.affectedRows) {
      return helper.getSuccessResponse(
        true,
        "Postprocessor updated successfully",
        admin.postprocessorid,
        ""
      );
    } else {
      return helper.getErrorResponse(
        false,
        "Error updating the Postprocessor.",
        "UPDATE POST PROCESSOR SERVER",
        ""
      );
    }
  } catch (error) {
    console.error("Error in UpdatePostProcessor:", error);
    return helper.getErrorResponse(
      false,
      "Internal error. Please contact Administration",
      error.message,
...
async function UpdateDeviceStart(admin) {
  ...
  // var logMessage;
  try {
    logMessage = `Device ID: ${admin.deviceid}, Status: ${
      admin.status
    },  startDate: ${
      admin.startdate
  ...
  // finally{
  //   const fs = require("fs");
  //     const logFilePath = path.join("C:\\", "device_start_log.txt");
...
async function DeleteAImachine(admin) {
  ...
  try {
    const sql = await db.query(
      "Update aisysmaster set status = 0 , deleted_flag = 1 where machine_id IN(?)",
      [admin.machineid]
    );
    if (sql.affectedRows) {
      return helper.getSuccessResponse(
...
async function UpdateDeviceStop(admin) {
  ...
  // finally{
  //   const fs = require("fs");
  //   const logFilePath = path.join("C:\\", "device_stop_log.txt");
  //   console.log("Log file path:", logFilePath);
  //   try {
  //     fs.appendFileSync(logFilePath, logMessage, "utf8");
  //   } catch (error) {
...
async function UpdateDeviceStartStop(admin) {
  if (admin.hasOwnProperty("deviceid") == false) {
    return helper.getErrorResponse(
      false,
      "Device id missing. Please provide the device id",
  ...
  try {
    ...
    console.log(logMessage);
    var sql;
    if (admin.startdate != null && admin.startdate != "") {
      [sql] = await db.spcall(`CALL SP_START_DEVICE_SERVICE(?,?,?);`, [
        admin.deviceid,
        admin.status,
        admin.startdate,
      ]);
    } else {
      [sql] = await db.spcall(`CALL SP_STOP_DEVICE_SERVICE(?,?,?,?);`, [
        admin.deviceid,
        admin.status,
        admin.logtext,
        admin.enddate,
...
//########################################################################################################################################################################################
//########################################################################################################################################################################################
async function RestartSite(admin) {
  if (admin.hasOwnProperty("siteid") == false) {
    return helper.getErrorResponse(
      false,
      "Site id missing. Please provide the site id",
  ...
  try {
    const sql = await db.query(
      `select manual_stop from branchmaster where branch_id = ${admin.siteid}`
    );
    if (sql[0]) {
      var status = sql[0].manual_stop;
      return helper.getSuccessResponse(true, "Success", status, "");
    } else {
      return helper.getErrorResponse(
        false,
        "Error while fetching the status",
        "FETCH SITE STATUS",
        ""
      );
    }
...
async function addeventruntime1(admin) {
  // console.log(JSON.stringify(admin));
  ...
  try {
    const [results] = await db.spcall(
      `CALL addeventruntime3_notigored(?,?,?,@camID,@eid); SELECT @camID AS camID, @eid AS eid;`,
      [admin.deviceid, admin.camerano, admin.alerttext]
    );
...
async function DashBoardCount(admin) {
  try {
    ...
    return {
      ...
      activedevice: counts.activedevice || 0,
      inactivedevice: counts.inactivedevice || 0,
...
//###################################################################################################################################################################################################
//###################################################################################################################################################################################################
async function UpdateECS(admin) {
  try {
    if (!admin.hasOwnProperty("deviceip")) {
      return {
        code: false,
        message: "Device ip address missing. Please provide the deviceip",
        module: "UPDATE ECS SYSTEM",
      };
    }
    if (!admin.hasOwnProperty("httpport")) {
      return {
    ...
    if (!admin.hasOwnProperty("ecsid")) {
      return {
        code: false,
        message: "ECS id missing. Please provide the ecs id.",
        module: "UPDATE ECS SYSTEM",
      };
    }
    ...
    console.log(`admin camera id -> ${admin.camera_id}`);
    try {
      const sql = await db.query(`UPDATE ecsmaster SET mac_address = '${
        admin.macaddress
      }',Extension = '${admin.extension}',device_ip = '${
        admin.deviceip
      }', http_port =  '${admin.httpport}',Serialnumber = '${
        admin.serialnumber
      }',Startplayurl = '${admin.playsoundurl}' , Stopplayurl =  '${
        admin.stopplayurl
      }',
            Ecstype =  '${admin.ecstype}',camera_id =  '${JSON.stringify(
        admin.camera_id
      )}',Site_id =  ${admin.siteid},
            Ecs_Username = '${admin.username} ', Ecs_password =  '${
        admin.password
      }' WHERE ecssystem_id = ${admin.ecsid};`);
      if (!sql.affectedRows) {
        return {
          code: false,
          message: "Error while adding the ecs system",
        ...
        return {
          code: true,
          message: "Emergency communication system Updated successfully.",
          ecsid: admin.ecsid,
        };
      }
    } catch (er) {
...
async function GetEcs(admin) {
  ...
  try {
    if (admin.siteid != "") {
      sql = await db.query(
        `select ecssystem_id ,Device_ip,http_port,mac_address,extension,serialnumber,startplayurl,stopplayurl,ecstype,camera_id ,Ecs_username username, Ecs_password passowrd from ecsmaster where site_id IN (${admin.siteid}) and status = 1 and deleted_flag =0;`
      );
      sql1 = await db.query(
...
async function DeleteEcs(admin) {
  if (admin.hasOwnProperty("ecsid") == false) {
    return helper.getErrorResponse(
      false,
      "ECS id missing. Please provide the ecs id.",
      "DELETE ECS SYSTEM",
      ""
    );
  }
  try {
    sql = await db.query(
      `Update ecsmaster set Deleted_flag = 1 ,status =0 where ecssystem_id = ${admin.ecsid};`
    );
    if (sql.affectedRows) {
      return {
        code: true,
        message: "Emergency communication system list deleted successfully",
        ecs: sql,
      };
    } else {
      return helper.getErrorResponse(
...
async function UpdateSiteController(admin) {
  ...
  const siteIds = Array.isArray(admin.siteid) ? admin.siteid : [admin.siteid];
  const responses = [];

  const copyOperations = siteIds.map(async (siteId) => {
    try {
      ...
      try {
        await fs.mkdir(sitecontrollerPath, { recursive: true });
        console.log("Folder created successfully.");
      } catch (error) {
        if (error.code !== "EEXIST") {
          console.error("Error creating folder:", error.message);
      ...
      const sourceFolder = config.sitecontrollerpath;
      try {
        await cpy.copy(sourceFolder, sitecontrollerPath);
        await addsiteconfig(siteId);
        console.log("Files copied successfully!");
        responses.push({
          siteid: siteId,
        ...
        responses.push({
          code: false,
          message: `Error while copying files for site id ${siteId}`,
          error: error.message,
        });
      }
    } catch (error) {
...
async function GetSiteControllerProcess(admin) {
  try {
    ...
    const sql = await db.query(
      `select Process_id from branchmaster where Branch_id =${admin.siteid} and status = 1`
    );
    if (sql[0]) {
      const processid = sql[0].Process_id;
      return {
...
async function SiteStatusUpdate(admin) {
  try {
    ...
    const [sql] = await db.spcall(
      `CALL SP_SITE_STATUS(?,?,?,?,?,?,?,?,?,?,@statusid); select @statusid`,
      [
        admin.sdkid,
        admin.siteid,
        admin.alertmessage,
    ...
    if (objectvalue["@statusid"] != null) {
      return helper.getSuccessResponse(
        true,
        "Site status Updated successfully",
        objectvalue["@statusid"],
        ""
      );
    } else {
      return helper.getErrorResponse(
...
async function WebSocketServer(admin) {
  try {
    const sql = await db.query(
      `select * from websocketmaster where status = 1`
    );
    if (sql[0]) {
      return helper.getSuccessResponse(true, "Success", sql);
    } else {
      return helper.getErrorResponse(false, "Failed", sql);
    }
  } catch (er) {
    return helper.getErrorResponse(
      false,
...
async function GetCameraPlacement(admin) {
  try {
    const sql = await db.query(
      `select * from cameraplacementmaster where status = 1`
    );
    if (sql[0]) {
      return helper.getSuccessResponse(true, "Success", sql);
    } else {
...
async function AddCameraPlacement(admin) {
  try {
    if (admin.hasOwnProperty("cameraid") == false) {
      return helper.getErrorResponse(
        false,
    ...
    if (admin.hasOwnProperty("placementncame") == false) {
      return helper.getErrorResponse(
        false,
        "Placement Name missing. Please provide the placement name.",
        "ADD CAMERA PLACEMENT",
        ""
      );
    }
    if (admin.hasOwnProperty("placementid") == false) {
      return helper.getErrorResponse(
        false,
        "Placement id missing. Please provide the placement id.",
        "ADD CAMERA PLACEMENT",
        ""
      );
    }
    const sql = await db.query(
      `UPDATE cameramaster SET display_name = ?,placement_id = ? WHERE Camera_id = ?`,
      [admin.placementname, admin.placementid, admin.cameraid]
    );
    if (sql.affectedRows) {
      return helper.getSuccessResponse(true, "Success", sql);
    } else {
...
async function FetchCoordinates(admin) {
  try {
    if (admin.hasOwnProperty("deviceid") == false) {
      return helper.getErrorResponse(
        false,
        "Device id missing. Please provide the Device id",
        "FETCH THE IMAGE COORDINATES",
        ""
      );
    }
    const sql = await db.query(
      `SELECT cm.camera_id, ec.coordinates FROM cameramaster cm LEFT JOIN  analyticcoordinates ec ON  cm.camera_id = ec.camera_id WHERE cm.Device_id = ${admin.deviceid} ORDER BY cm.camera_id`
    );
    if (sql[0]) {
      return helper.getSuccessResponse(true, "Success", sql);
    } else {
...
async function EndoftheCode(admin) {
  if (admin.hasOwnProperty("Siteid") == false) {
    return helper.getErrorResponse(
      false,
      "Site id missing. Please provide the site id",
  ...
  if (admin.hasOwnProperty("sitename") == false) {
    return helper.getErrorResponse(
      false,
      "Site name missing. Please provide the site name",
      "Site name missing",
      ""
    );
  }
  if (admin.hasOwnProperty("Department") == false) {
    return helper.getErrorResponse(
      false,
      "Department name missing. Please provide the department name",
      "Department name missing",
      ""
    );
  }
  if (admin.hasOwnProperty("locationname") == false) {
    return helper.getErrorResponse(
      false,
      "Location name missing. Please provide the Location name",
      "Location name missinf",
      ""
    );
  }
  // if(admin.)
...
async function SiteDeactivate(admin) {
  try {
    if (admin.hasOwnProperty("siteid") == false) {
      return helper.getErrorResponse(
        false,
    ...
    // const original_id = helper.hashSiteId(admin.siteid,config.sitesalt);
    const [result] = await db.spcall(
      `CALL SP_SITE_MODIFY(?,?,@result); SELECT @result;`,
      [admin.siteid, 0]
    );
    ...
    if (status !== undefined && status !== null && status !== 0) {
      ...
      return `Error while Deactivating site ${admin.siteid}`;
    }
  } catch (er) {
    console.log(er);
    return helper.getErrorResponse(
...
async function SiteDeletes(admin) {
  try {
    if (admin.hasOwnProperty("siteid") == false) {
      return helper.getErrorResponse(
        false,
    ...
    if (status !== undefined && status !== null && status !== 0) {
      ...
      return `Error while Deleted site ${admin.siteid}`;
    }
  } catch (er) {
    console.log(er);
    return helper.getErrorResponse(
...
async function DemoSite(admin) {
  try {
    if (admin.hasOwnProperty("siteid") == false) {
      return "Site id missing. Please provide the Site id";
    }
    if (admin.hasOwnProperty("action") == false) {
      return "Action missing. Please provide the action";
    }
    const result = await db.query(
      `SELECT to_date FROM subscriptioncustomertrans WHERE branch_id = ? ORDER BY to_date DESC LIMIT 1`,
      [admin.siteid]
    );

    if (result.length === 0) {
      return "No active subscription found for the provided Site ID.";
    }

    const currentToDate = new Date(result[0].to_date);
    const newToDate = new Date(currentToDate);
    newToDate.setMonth(newToDate.getMonth() + 1); // Extend by 1 month

    if (admin.action == "extend") {
      // Update the to_date in the database
      await db.query(
        `UPDATE subscriptioncustomertrans SET to_date = ? WHERE branch_id = ? AND to_date = ?`,
        [newToDate, admin.siteid, currentToDate]
      );

      return `Subscription extended until ${
        newToDate.toISOString().split("T")[0]
      }`;
    } else {
      return `Site will be deactivated on ${
        currentToDate.toISOString().split("T")[0]
      }.`;
    }
  } catch (er) {
    console.log(er);
